...
class GraphViewModel<E, V>(
    ...
    fun onVertexPopupConfirm(label: V, x: Dp, y: Dp) {
    ...
    fun onEdgePopupConfirm(element: E) {
        val first = _focusedVertex ?: return
        val second = _secondVertex ?: return
        addEdgeViewModel(first, second, element)
        first.unFocus()
        _focusedVertex = null
        _secondVertex = null
        showEdgePopup = false
    ...
    fun addVertexViewModel(label: V, x: Dp, y: Dp, color: Color) {
    ...
    fun addEdgeViewModel(
        ...
        val edge = graph.addEdge(focused.label, current.label, e)
    ...
    fun highlightBridges() {
        ...
        val algorithm = FindBridges(graph as UndirectedGraph)
